...
int taskCurrent = 0xFF; // Maximum size of Queue to put processes waiting for Semaphore
int taskCount = 0;      // total number of valid tasks
unsigned int stTime = 39999; // stTime and endTime to store the process execution time of ech task
unsigned int endTime = 0;
int cpuTimeout = 1000;      // CpuTimeout to acknowledge caluculation of processor Usage calculation
bool PI = false;            // PrirotyInheritance Selector
int zeros = 0;      // Variable to count how many times the timer overflowed in a particular process execution
bool flag=0;

struct semaphore 
{
  unsigned int count;   // Count of the semaphore
  unsigned int queueSize;   // Current queuesize (no.of Processes waiting in queue)
  unsigned int processQueue[MAX_QUEUE_SIZE]; // store task Pid
  char name[15];        // name of the semaphore
  unsigned int readIndex;   // Read and write Indexes from the processQueue to make it circular queue
  unsigned int writeIndex;
  int currentUser;      // current user of semaphore at the current instance (PID of process)
}*s, lock, pInheritance;
...
void wait(void * pSemaphore) 
{
  ...
  if (s -> count == 0) 
  {   if(s-> currentUser != 0xff){
   if ( (tcb[s -> currentUser].priority > tcb[taskCurrent].priority))
     {
...
void post(void * pSemaphore) 
{
 ...
 if(taskCurrent!=0xff){
  ...
  s -> currentUser = 0xFF;
    if (taskCurrent!=0xff && tcb[taskCurrent].currentPriority != tcb[taskCurrent].priority) {
...
{
  int i;
  taskCount = 0;    // No tasks Running or Created (At start of program)
...
int rtosScheduler() 
{
  ...
  {  
    ...
    {
      task = 0;
      printf("many tasks, resetting to 0\n");
    }
...
__asm("addi a7,x0,17");
  ...
  char task[15];
  strcpy(task, "task1");
  bool ox = createProcess(myTask1, 2, task);
  // printf("%d\n",*myTask1);
  //printf("\n^^^^^^^^^^^^^^\n");
  //printf("in main %d\n %s\n",1,tcb[0].name);
  // destroyProcess(myTask1,task);
  strcpy(task, "task2");
  bool ol = createProcess(myTask2, 1, task);
  //printf("in main %d\n %s\n",2,tcb[1].name);
  strcpy(task, "task3");
  bool op = createProcess(myTask3, 3, task);
  printf("\n*************************************\n");
