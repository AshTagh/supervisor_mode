...
int readbytes(i2c_struct * i2c_instance, char *buf, int count, int last)
{
	...
	for (i = 0; i <= count; i++) {
		...
		if (i)
		{
			...
			i2c_instance->data = !i2c_instance->data; /* dummy read */
 ...
 */
int i2c_read_interrupt_data(i2c_struct * i2c_instance, unsigned char *read_data, unsigned char delay, unsigned char last)
{
	int status = 0;
