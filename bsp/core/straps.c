...
mtrap_fptr_t scause_trap_table[MAX_TRAP_VALUE];
mtrap_fptr_t scause_interrupt_table[MAX_INTERRUPT_VALUE];
 ...
 */
void sdefault_handler(__attribute__((unused)) uintptr_t scause, __attribute__((unused)) uintptr_t epc)
{
 ...
 */
uintptr_t shandle_trap(uintptr_t scause, uintptr_t epc)
{
	unsigned int ie_entry = 0;;
	uint32_t shift_length = 0;

	//log_trace("\nshandle_trap entered\n");
	...
	if (scause & (1 << (shift_length))){

		ie_entry = extract_ie_code(scause);

		log_debug("Source of Trap: Interrupt\n");

		scause_interrupt_table[ie_entry](scause, epc);
	}
	else{
		log_debug("Source of Trap: Software\n");

		scause_trap_table[scause](scause, epc);
	}
